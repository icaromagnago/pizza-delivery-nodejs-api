...
// cardName, cardNumber, cardExpireMonth, cardExpireYear and cardCvc
handlers._orders.post = (data, callback) => {
  ...
  if (!helpers.isStringNotEmpty(cardExpireMonth)) {
    validationErrors.cardExpireMonth = 'expiration month is required';
  }
  if (!helpers.isStringNotEmpty(cardExpireYear)) {
    validationErrors.cardExpireYear = 'expiration year is required';
  }
      ...
      if (tokenIsValid) {
        _data.read('cart', cartId, (err, cartData) => {
          if (!err) {
            ...
            helpers.sendStripePayment(payloadPayment, (err) => {
              if (!err) {
